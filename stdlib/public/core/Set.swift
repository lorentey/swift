...
extension _NativeSet {
  /// Returns true iff contents were rehashed.
  @usableFromInline
  internal mutating func reallocate(capacity: Int) -> Bool {
  ...
  internal mutating func ensureUnique(
    ...
    let rehashed = reallocate(capacity: capacity)
...
extension _NativeSet: _SetBuffer {
  ...
  internal func element(at i: Index) -> Element {
    _storage.hashTable.checkOccupied(i)
    defer { _fixLifetime(self) }
    return elements[i.bucket]
  }
}
...
extension _CocoaSet: _SetBuffer {
  ...
  @inlinable
  internal func contains(_ object: AnyObject) -> Bool {
